...
class StatArb:
    ...
    - ``self.signal`` (pd.DataFrame) Signals to Long/Short asset.
    ...
    def allocate(self, price_x, price_y, window=0, intercept=True):
        ...
        # Convert z_score.
        self._convert_z_score(self.z_score)
    ...
    def _rolling_allocate(self, np_x, np_y):
        ...
        # Columns name.
        col_name = [data_x.name, data_y.name, 'logret_x', 'logret_y', 'beta', 'constant',
                    'ret_spread',
                    'cum_resid', 'z_score']
        return pd.DataFrame(res, index=data_x.index, columns=col_name)


    def _convert_z_score(self, z_score):
        """
