...
class TestDataStructures(unittest.TestCase):
    ...
    def test_ema_imbalance_dollar_bars(self):
        ...
        self.assertEquals(db1.loc[0, 'open'], 1306.0)
        self.assertEquals(db1.loc[0, 'high'], 1306.0)
        self.assertEquals(db1.loc[0, 'low'], 1304.25)
        self.assertEquals(db1.loc[0, 'close'], 1304.5)

        self.assertTrue((db1.loc[:, 'high'] >= db1.loc[:, 'low']).all())

        # Assert OHLC is correct (some index)
        self.assertEquals(db1.loc[1387, 'open'], 1303.5)
        self.assertEquals(db1.loc[1387, 'high'], 1303.5)
        self.assertEquals(db1.loc[1387, 'low'], 1303.5)
        self.assertEquals(db1.loc[1387, 'close'], 1303.5)
    ...
    def test_ema_imbalance_tick_bars(self):
        ...
        self.assertEquals(db1.loc[0, 'high'], 1306.0)
        self.assertEquals(db1.loc[0, 'low'], 1304.25)
        self.assertEquals(db1.loc[0, 'close'], 1304.25)

        # Assert OHLC is correct (some index)
        self.assertEquals(db1.loc[1387, 'open'], 1308.0)
        self.assertEquals(db1.loc[1387, 'high'], 1308.0)
        self.assertEquals(db1.loc[1387, 'low'], 1308.0)
        self.assertEquals(db1.loc[1387, 'close'], 1308.0)
        self.assertTrue((db1.loc[:, 'high'] >= db1.loc[:, 'low']).all())
        self.assertTrue((db1.loc[:, 'volume'] >= db1.loc[:, 'cum_buy_volume']).all())
