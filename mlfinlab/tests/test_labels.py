...
from mlfinlab.labeling import get_trend_scanning_labels
...
class TestChapter3(unittest.TestCase):
    ...
    def test_trend_scanning_labels(self):
        ...
        self.assertTrue(
            set(trend_scanning_labels.loc[pd.Timestamp(2008, 1, 1):pd.Timestamp(2008, 5, 12)].bin) == set([1]))

        self.assertEqual(trend_scanning_labels.bin.value_counts()[-1], 71)  # Number of -1 labels check
        self.assertEqual(trend_scanning_labels.bin.value_counts()[1], 39)

        # Approx. 20 latest labels are nan because we don't have data for look-forward period (20)
        self.assertEqual(trend_scanning_labels.shape[0] - trend_scanning_labels.dropna().shape[0], 19)

        trend_scanning_labels.dropna(inplace=True)  # Drop na values

        # Index should be < t1
        self.assertTrue((trend_scanning_labels.t1 > trend_scanning_labels.index).all())

        for int_index, (ret_v, bin_v) in zip([0, 2, 10, 20, 50],
                                             [(0.04837, 1), (0.03763, 1), (0.05866, 1), (0.05309, 1), (-0.0812, -1)]):
            self.assertAlmostEqual(trend_scanning_labels.iloc[int_index]['ret'], ret_v, delta=1e-4)
            self.assertEqual(trend_scanning_labels.iloc[int_index]['bin'], bin_v)
