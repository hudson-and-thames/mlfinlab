...
def get_roll_measure(close_prices, window):
    ...
    return 2 * np.sqrt(abs(price_diff.rolling(window=w).cov(price_diff_prev)))
...
def get_yang_zhang_vol(open, close, window):
