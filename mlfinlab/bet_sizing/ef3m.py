...
class M2N:
    ...
    def fit(self, mu2, epsilon, variant=1, maxIter=100_000):
        ...
        :param mu2: (float) An initial estimate for the mean of the second distribution.
        :param epsilon: (float) Error tolerance.
        :param variant: (int) Which algorithm variant to use, 1 or 2.
        :param maxIter: (int) Maximum number of iterations after which to terminate loop.
        """
        ...
        while True:
            numIter += 1
            if variant == 1:
                parameters_new = self.iter_4(mu2, p1, self.moments)  # first variant
            elif variant == 2:
                parameters_new = self.iter_5(mu2, p1, self.moments)  # second variant
            else:
    ...
    def get_moments(self, parameters):
        ...
        :param parameters: (list) List of parameters if the specific order [mu1, mu2, sigma1, sigma2, p1]
        ...
        m4 = p1*(3*s1**4 + 6*s1**2*u1**2 + u1**4) + p2*(3*s2**4 + 6*s2**2*u2**2 + u2**4)  # Eq. (9)
        m5 = p1*(15*s1**4*u1 + 10*s1**2*u1**3 + u1**5) + p2*(15*s2**4*u2 + 10*s2**2*u2**3 + u2**5)  # Eq. (10)
        return [m1, m2, m3, m4, m5]

    def iter_4(self, mu2, p1, moments):
        """
        ...
        :param moments: (list) First five raw moments of the mixture distribution [m1, m2, m3, m4, m5]
        ...
        if (3*(1-p1)*(mu2-mu1)) == 0:
            ...
            return []
        sigma2_squared = ( (m3 + 2*p1*mu1**3 + (p1-1)*mu2**3 - 3*mu1*(m2 + mu2**2*(p1-1))) / (3*(1-p1)*(mu2-mu1)) )
        if sigma2_squared < 0:
            return []
        sigma2 = sigma2_squared**(.5)
        # sigma1, Equation (23)
        sigma1_squared = ( (m2 - sigma2**2 - mu2**2)/p1 + sigma2**2 + mu2**2 - mu1**2 )
        if sigma1_squared < 0:
            return []
        ...
        p1_deno = (3*(sigma1**4 - sigma2**4) + 6*(sigma1**2*mu1**2 - sigma2**2*mu2**2) + mu1**4 - mu2**4)
        if p1_deno == 0:
    ...
    def iter_5(self, mu2, p1, moments):
        """
        ...
        if np.iscomplex(sigma1) or np.iscomplex(sigma2) or np.isnan(sigma1) or np.isnan(sigma2):
            return []
        # adjust the guess for mu2, Equation (27)
        if (1-p1) < 1e-4:
            return []
        a = ( 6*sigma2**4 + (m4-p1*(3*sigma1**4+6*sigma1**2*mu1**2+mu1**4)) / (1-p1 ) )**.5
    ...
    def single_loop(self, moments, epsilon=10**-5, factor=5,
                    variant=1, maxIter=100_000):
        """
        A single scan through the list of mu2 values, cataloging the successful fittings in a DataFrame.
        ...
        :param maxIter: (int) Maximum number of iterations to perform in the 'fit' method
        :return: (pd.DataFrame) Fitted parameters and error
        """
        stDev = centered_moment(moments, 2)**.5
        ...
        for mu2_i in mu2:
            ...
            if m2n.error < err_min:
                err_min = m2n.error
                d_results['mu1'], d_results['mu2'], d_results['sigma1'], d_results['sigma2'], d_results['p1'] = [[p] for p in m2n.parameters]
                d_results['error'] = [err_min]
        return pd.DataFrame.from_dict(d_results)

    # repeat runs and collect results as a DataFrame
    def mp_fit(self, moments, epsilon=10**-5, factor=5, n_runs=1, variant=1,
                maxIter=100_000):
        """
        ...
        dfs = [delayed(self.single_loop)(moments=moments, epsilon=epsilon, factor=factor, variant=variant, maxIter=maxIter ) for i in range(n_runs)]
