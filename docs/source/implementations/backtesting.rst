...
3. This :math:`N+1` t-statistics are transformed again to p-values, taking into account the
data mining adjustment.
...
5. The steps 2-4 are repeated multiple times
(simulations).
...
the haircuts can be calculated.
...
3. Using two methods (Holm and BHY) we calculate the threshold t-statistic
that matches the :math:`p` significance level.

4. The steps 2-3 are repeated multiple times
(simulations).
