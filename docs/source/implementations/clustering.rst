...
Clustering is a task of grouping a set of data points into several groups where each group contains more similar data points compared to those in other groups. There are many known clustering categories, and some of the well-known ones are the distribution-based method, centroid based method, connectivity-based method, density model, and subspace clustering. One of the most popular clusterings is the K-means algorithm, which falls under the centroid based method category. K-means algorithm has an advantage that it guarantees its convergence in a finite number of steps; thus, it gives an assurance of a stable result. However, the K-means algorithm requires the user to set the number of clusters in advance. Furthermore, the initialization of the clusters is random. As consequent, the solution is often not optimal in some sense, and the effectiveness of the algorithm can be random.
...
This algorithm results in a tuple that contains an optimized correlation matrix, optimized clusters, and the Silhouette Scores of every node in the clusters.
...
13.) Assign new values of the correlation matrix, clustering, and Silhouette Scores to new variables. We obtain these values by applying the makeNewOutputs function to the original correlation matrix, clusters with keys that are not in redoClusters, and the sub-par clusters that have gone through the filtering process.
