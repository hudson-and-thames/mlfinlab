...
Initially, we feed an observation matrix into the algorithm. After that, the algorithm evaluates the observation matrix. Next, the Base Clustering performs a double for..loop. The first loop clusters the matrix for different *k* values from 2 to N via k-means for one given initialization and assesses the quality *q* for each clustering. The second loop redoes the first loop clustering multiple times until it satisfies the condition of different initializations. Lastly, the algorithm chooses the clustering with the highest quality *q* over these two loops.  
